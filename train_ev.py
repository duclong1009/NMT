...
def main(arg):
    seed_all(arg.seed)
    train_filepaths = [arg.root_path + "train.en", arg.root_path + "train.vi"]
    val_filepaths = [arg.root_path + "dev.en", arg.root_path + "dev.vi"]
    test_filepaths = [
        arg.root_path + "test.en",
        arg.root_path + "test.vi",
    ]
    en_tokenizer = get_tokenizer("spacy", language="en")
    vi_tokenizer = VnCoreNLP(
        arg.path_vncore, annotators="wseg", max_heap_size="-Xmx500m"
    )
    vi_vocab = build_vocab(train_filepaths[1], vi_tokenizer, True)
    en_vocab = build_vocab(train_filepaths[0], en_tokenizer)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    train_data = data_process(
        train_filepaths, en_vocab, vi_vocab, en_tokenizer, vi_tokenizer, 64, 64, device
    )
    val_data = data_process(
        val_filepaths, en_vocab, vi_vocab, en_tokenizer, vi_tokenizer, 64, 64, device
    )
    test_data = data_process(
        test_filepaths, en_vocab, vi_vocab, en_tokenizer, vi_tokenizer, 64, 64, device
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--path_vncore", type=str, default="./vncorenlp/VnCoreNLP-1.1.1.jar"
    )
    parser.add_argument("--root_path", type=str, default="./data/en2vi/")
